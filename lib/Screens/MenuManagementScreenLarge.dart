          ...
          if (_selectedCategoryId != null)
            ElevatedButton.icon(
              ...
              onPressed: () => showDialog(
                context: context,
                builder: (ctx) {
                  // Pre-fill category ID if possible (MenuItemDialog handles defaults via doc, but for new item we might want to pass it?
                  // MenuItemDialog checks doc. For new item, it defaults to null.
                  // Ideally we should pass the selected CategoryId to the dialog so it pre-selects it.
                  // I will update MenuItemDialog in the future to accept initialCategoryId, but for now user can select it manually.
                  return const MenuItemDialog();
                },
              ),
  ...
  Widget build(BuildContext context) {
    ...
    if (!userScope.isSuperAdmin) {
      if (userScope.branchId != null) {
        ...
        return const Center(child: Text('No branch assigned'));
      }
    }
...
class _MenuItemsGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (!userScope.isSuperAdmin && userScope.branchId != null) {
      // For items, we usually just filter by category,
      // but strictly speaking we might want to check if the item is 'assigned' to this branch.
      // The original code filtered by branchIds containing userScope.branchId.
      query = query.where('branchIds', arrayContains: userScope.branchId);
    }
        ...
        return GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return PremiumMenuItemCard(
              ...
              onEdit: () => showDialog(
                context: context,
                builder: (ctx) => MenuItemDialog(doc: doc),
              ),
